...
| Detector 邊界 | `python examples/run_detector_evaluation.py` | 24 個 synthetic 案例：precision `1.000`、recall `0.500`、零 false positive |
...
review 只寫 `.codex-metabolism/`；同一專案持續使用相同 output directory，讓 `interventions.jsonl` 串接批准與後續證據。
